...
## Alur Transaksi dan Chain {#alur-transaksi-blockchain}
...
4. Miner melakukan [mining](#mining-proof-of-work) (di [PoS](#proof-of-stake) validator melakukan [minting](#minting-proof-of-stake)), yaitu proses membentuk block baru dan menambahkan block itu ke jaringan
5. Setelah [mining](#mining-proof-of-work)/[minting](#minting-proof-of-stake) berhasil dilakukan (mendapatkan hash yang valid), nonce disimpan di candidate block (yang sekarang sudah menjadi block yang sah). Miner menyebarkan block baru ke seluruh [jaringan](#distributed-element), dan node-node lain memverifikasi apakah hash valid dan seluruh transaksi di block valid dan belum pernah digunakan. Jika valid, block diterima dan ditambahkan ke chain. Miner mendapatkan block reward (awal-awal 50 BTC, tapi setiap halving dibagi 2, sekarang kira-kira 3.125 BTC), plus semua transactions fees dalam block tersebut
...
## Distributed Element {#distributed-element}
...
| Mining/minting node   | Menjalankan fungsi full node plus proses [mining](#mining-proof-of-work) atau [minting](#minting-proof-of-stake)                                        | Mengonsumsi daya tinggi, biasanya bergabung dalam suatu pool                    |

- Penjelasan mengenai recovery dilihat di [konsensus](#konsensus-blockchain)
...
# Consensus {#konsensus-blockchain}
...
### Conflict Resolution {#conflict-resolution-proof-of-work}
...
### Recovery {#recovery-proof-of-work}
...
- <b><term href="/glossary#finality">Finality</term></b> adalah keadaan ketika suatu blok dianggap tidak dapat dibatalkan atau digantikan oleh cabang lain. Setelah blok mencapai finality, seluruh transaksi di dalamnya dianggap permanen dan tidak bisa di-reorganize.
...
### Minting/Forging {#minting-proof-of-stake}
...
4. Reward/penalty: jika validator jujur, maka validator menerima reward setelah berhasil membuat block baru (biasanya proporsional terhadap stake/koin yang disimpannya). Jika tidak jujur, maka menerima [penalti](#penalti-proof-of-stake)
5. Consensus: block disebarkan ke seluruh jaringan peer-to-peer

### Penalti Kecurangan {#penalti-proof-of-stake}
...
## Consensus Lain {#konsensus-lain-blockchain}
...
- Contoh: Solana
- PoH bukan mekanisme konsensus tunggal, tetapi komponen pendukung untuk algoritma [Proof-of-Stake](#proof-of-stake) Solana.
...
```solidity
using SafeMath for uint256; uint256 public totalSupply;
```
...
### Karakteristik {#karakteristik-dapps}
...
- Smart contract – dApps menggunakan [smart contract](#smart-contract-blockchain) untuk mengotomatiskan proses dan mengeksekusi transaksi. Smart contract adalah kontrak yang dapat dieksekusi sendiri dengan ketentuan perjanjian yang tertulis langsung ke dalam kode.
- Mekanisme Insentif – Banyak dApps memiliki mekanisme insentif bawaan, yang seringkali melibatkan [token](#token-blockchain), untuk memberi penghargaan kepada pengguna dan kontributor atas partisipasi dan upaya mereka.
...
### Benefit {#benefit-dapps}

- Transparansi – Semua transaksi dan operasi dicatat dalam public [ledger](#dlt), memberikan transparansi dan mengurangi risiko penipuan.
...
- Trustless environment – Pengguna tidak perlu mempercayai otoritas pusat atau perantara, karena blockchain dan [smart contract](#smart-contract-blockchain) memastikan eksekusi transaksi yang benar.
...
- LMD-GHOST memastikan semua validator secara cepat konvergen pada satu rantai dengan bobot dukungan terbanyak, sambil [Casper FFG](#casper-ffg) menjamin finality di atasnya.

### Casper FFG {#casper-ffg}

- Protokol [finality](#proof-of-stake) yang digunakan Ethereum PoS
- Mekanisme:
...
4. Jika validator menandatangani dua vote yang saling bertentangan → [slashing](#penalti-proof-of-stake).
...
- Jika [blockchain](#blockchain) menggunakan chain of blocks, IOTA menggunakan [directed acyclic graph]({{ "/discrete-math#graf-asiklik" | relative_url }})
...
- Backend logic (smart contract) contohnya bisa dilihat di [smart contract](#smart-contract-blockchain)
