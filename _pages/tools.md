---
layout: page-with-toc
title: Tools
description: "List of useful tools. Beware of virus or malicious contents"
permalink: "/tools/"  
github_edit_url: https://github.com/bagassambega/PersonalNotes/edit/main/_pages/tools.md
---
# Software Store

## Windows

| Name   | URL                                      | Description                     |
| ------ | ---------------------------------------- | ------------------------------- |
| FileCR | [https://filecr.com](https://filecr.com) | Download premium apps and games |

## Android

| Name     | URL                                                        | Description                                       |
| -------- | ---------------------------------------------------------- | ------------------------------------------------- |
| Mobilism | [https://forum.mobilism.org/](https://forum.mobilism.org/) | Download premium apps and games. Account required |


# Software Tools

## File Manipulation

| Name                  | Functionality                                    | Compatibility  | Source                                                                                               |
| --------------------- | ------------------------------------------------ | -------------- | ---------------------------------------------------------------------------------------------------- |
| zbarimg               | Scan QR code in an image                         | Linux          | [https://github.com/mchehab/zbar](https://github.com/mchehab/zbar)                                   |
| yt-dlp                | Audio/video downloader                           | Linux, Windows | [https://github.com/yt-dlp/yt-dlp](https://github.com/yt-dlp/yt-dlp)                                 |
| exiftool              | Manipulate file metadata, especially image       | Linux, Windows | [https://github.com/exiftool/exiftool](https://github.com/exiftool/exiftool)                         |
| GraphicsMagick        | Forks of ImageMagick, for edit, manipulate image | Linux          | [http://www.graphicsmagick.org/](http://www.graphicsmagick.org/)                                     |
| Calibre Ebook Convert | Convert EPUB or any other file into PDF          | Linux          | [https://man.archlinux.org/man/ebook-convert.1.en](https://man.archlinux.org/man/ebook-convert.1.en) |

# Diagram and Notes


| Name                 | Functionality                                | Platform        | URL                                               |
| -------------------- | -------------------------------------------- | --------------- | ------------------------------------------------- |
| Excalidraw           | Whiteboard tool                              | Web and PWA     | [https://excalidraw.com](https://excalidraw.com/) |
| AFFINE               | Knowledge based tool for docs and whiteboard | Web and Desktop | [https://app.affine.pro](https://app.affine.pro/) |
| Microsoft OneNote    | Knowledge based tool for docs and whiteboard | Desktop         | Office 365                                        |
| Microsoft Whiteboard | Whiteboard tool                              | Desktop         | Microsoft Windows                                 |


# Article, Journal and Book

| Name            | URL                                                                                                   | Description                                                                                              |
| --------------- | ----------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| Freedium        | [https://freedium.cfd](https://freedium.cfd)                                                          | Unlock premium Medium articles                                                                           |
| SciHub          | [https://sci-hub.se/](https://sci-hub.se)<br>[https://sci-hub.ru/](https://sci-hub.ru/)               | Unlock journals or scientific articles                                                                   |
| Libgen          | [https://libgen.li](https://libgen.li)                                                                | Access journals and books                                                                                |
| Annas-archive   | [https://annas-archive.gl](https://annas-archive.gl/)                                                 | Access books                                                                                             |
| zlib            | [https://z-library.bz/](https://z-library.bz/)                                                        | Access books                                                                                             |
| Unpaywall       | [https://unpaywall.org/](https://unpaywall.org/)                                                      | Access journal or scientific articles                                                                    |
| OceanofPDF      | [https://oceanofpdf.com/](https://oceanofpdf.com/)                                                    | Access books                                                                                             |
| Perplexity      | [https://www.perplexity.ai/](https://www.perplexity.ai/)                                              | AI tools best for finding journal (use research and change mode to academic, disable web)                |
| Elicit          | [https://elicit.com/](https://elicit.com/)                                                            | AI tools for finding journal                                                                             |
| Litmaps         | [https://www.litmaps.com/](https://www.litmaps.com/)                                                  | AI tools for finding journal and their correlation with other journals by finding citation (graph-based) |
| IEEE Xplore     | [https://ieeexplore.ieee.org/](https://ieeexplore.ieee.org/)                                          | Tools to find IEEE journal                                                                               |
| CitationMachine | [https://www.citationmachine.net/bibtex](https://www.citationmachine.net/bibtex)                      | Tools to generate BibTex citation                                                                        |
| GetBibtex       | [https://www.getbibtex.com](https://www.getbibtex.com/)                                               | Tools to generate BibTex citation from URL/website                                                       |
| BibTex.com      | [https://www.bibtex.com/c/doi-to-bibtex-converter](https://www.bibtex.com/c/doi-to-bibtex-converter/) | Tools to generate BibTex citation                                                                        |

# Learning, Tutorial, and Certification


| Name            | URL                                                                      | Description                                                                                                           |
| --------------- | ------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------- |
| Microsoft Learn | [https://learn.microsoft.com/en-us/](https://learn.microsoft.com/en-us/) | Microsoft platform to learning about AI, system design, and Microsoft product                                         |
| Dicoding        | [https://www.dicoding.com/](https://www.dicoding.com/)                   | Indonesian based company for learning and getting certification about programming language, CS skills, and frameworks |
| Scrimba         | [https://scrimba.com/](https://scrimba.com/)                             | Learning and getting certification about programming language and doing live coding directly on the website           |
| Netacad         | [https://www.netacad.com/](https://www.netacad.com/)                     | Learning and getting certification about network, cybersecurity, and system design from Cisco                         |
| Fortinet        | [https://training.fortinet.com/](https://training.fortinet.com/)         | Learning and getting certification about network and cybersecurity from Fortinet                                      |
| Alkademi        | [https://alkademi.id](https://alkademi.id)                               | Learning and getting certification about programming in Indonesia                                                     |
| EdX             | [https://learning.edx.org/](https://learning.edx.org/)                   | Learning and getting certification from many large companies                                                          |
| MIT             | [https://ocw.mit.edu/](https://ocw.mit.edu/)                             | Learning free video course                                                                                            |

# Server, Database and Data Storage

| Name          | URL                                                            | Descrsiption                                                                                                                                  |
| ------------- | -------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| Imgur         | [https://imgur.com/](https://imgur.com/)                       | Free image storage (public, don't upload personal/private items)                                                                              |
| Azure         | [https://portal.azure.com](https://portal.azure.com)           | Microsoft server hosting and cloud provider (PaaS)                                                                                            |
| Cloudflare    | [https://dash.cloudflare.com/](https://dash.cloudflare.com/)   | Hosting, server, cloud, DNS, and DDoS prevention system                                                                                       |
| MongoDB Atlas | [https://www.mongodb.com/](https://www.mongodb.com/)           | MongoDB online databases provider. Free tier available                                                                                        |
| Railway       | [https://railway.com/](https://railway.com/)                   | Project hosting provider. Well-known for its database hosting. Free tier available                                                            |
| Vercel        | [https://vercel.com/](https://vercel.com/)                     | Frontend hosting provider. Optimized for NextJS. Free tier available                                                                          |
| DigitalOcean  | [https://www.digitalocean.com/](https://www.digitalocean.com/) | Server and cloud provider                                                                                                                     |
| Oracle        | [https://www.oracle.com/](https://www.oracle.com/)             | Oracle server and cloud provider. Free tier available at [https://www.oracle.com/asean/cloud/free/](https://www.oracle.com/asean/cloud/free/) |
| Netlify       | [https://app.netlify.com/](https://app.netlify.com/)           | Hosting frontend and serverless backend. Free tier available                                                                                  |
| Convex        | [https://www.convex.dev/](https://www.convex.dev/)             | Backend for typescript hosting. Free tier available                                                                                           |
| Supabase      | [https://supabase.com/](https://supabase.com/)                 | PostgreSQL online database provider. Free tier available                                                                                      |
| AppWrite      | [https://appwrite.io/](https://appwrite.io/)                   | Cloud provider. Free tier available                                                                                                           |
| Coolify       | [https://coolify.io/](https://coolify.io/)                     | Hosting platform and simple cloud services PaaS. Free tier available                                                                          |
| Free For Dev  | [https://free-for.dev](https://free-for.dev/)                  | List of free resources for IT developer                                                                                                       |

# Entertainment

| Name        | URL                                                                        | Genre             | Description                                                                  |
| ----------- | -------------------------------------------------------------------------- | ----------------- | ---------------------------------------------------------------------------- |
| fmhy        | [https://fmhy.net](https://fmhy.net)                                       | List of resources | List of free and (probably) pirated resources                                |
| PStream     | [https://pstream.mov](https://pstream.mov)                                 | Movie             | Streaming movies                                                             |
| Aether      | [https://aether.mom](https://aether.mom)                                   | Movie             | Streaming movies                                                             |
| Comix       | [https://comix.to](https://comix.to)                                       | Manga comic       | Manga reading, replacement for Comick                                        |
| Anikai      | [https://anikai.to](https://anikai.to)                                     | Anime             | Watch anime                                                                  |
| NontonAnime | [https://s7.nontonanimeid.boats/](https://s7.nontonanimeid.boats/)         | Anime             | Watch anime (Indonesian subtitle)                                            |
| AllManga    | [https://allmanga.to](https://allmanga.to/)                                | Anime & manga     | Watch anime and manga                                                        |
| ani-cli     |                                                                            | Anime             | CLI based for anime watch                                                    |
| PlayIMDB    | [https://www.playimdb.com/title/{id}](https://www.playimdb.com/title/{id}) | Movie             | Go to any movie with IMDB, modify the URL by changing `imdb` with `playimdb` |
| mkissa      | [https://mkissa.to](https://mkissa.to)                                     | Anime & manga     | Watch anime and manga                                                        |
| idlix       | [https://z2.idlixku.com/](https://z2.idlixku.com/)                         | Movie             | Streaming movies                                                             |
